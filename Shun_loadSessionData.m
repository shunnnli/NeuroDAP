...
withCamera = ~isempty(dir(fullfile(session.path,'cam*.avi')));
...
% session.pathNidq = strcat(session.path, '\catgt_',sessionName);
...
if withPhotometry; session.pathPhotometry = strcat(session.path,'\Photometry','\'); end
...
raw = double([allTones; leftTone; rightTone;...
    ...
    blueLaser; redLaser; basket(1,:); basket(2,:); basket(3,:); ...
...
if ni_photometryON
    save(strcat(session.path,'\','sync_',sessionName),...
        ...
        'leftSolenoid','rightSolenoid','allTones','temperature',...
...
if withRecording
    ...
    %% Save ephys data
    save(strcat(session.path,'\','sync_',sessionName),'ap','-append');
end
...
if withPhotometry
    ...
    params.photometry.inclFreqWin = 4; % Number of frequency bins to average (on either side of peak freq)
    ...
    params.photometry.spectralWindowNew = 20;
...
if withRecording; imec_idx = find(syncImec_diff>0,syncPulseWindow); end
...
if withRecording; ISI_imec = (imec_idx(2:end) - imec_idx(1:end-1)) / params.sync.apFs; end
...
if withRecording; [r_imec, l_imec] = xcorr(zscore(ISI_ni), zscore(ISI_imec),'normalized'); end

% 4. Find the max of xcorr
CamNI_ni = 0; LJNI_ni = 0; ImecNI_ni = 0;
CamNI_niIdx = 0; LJNI_niIdx = 0; ImecNI_niIdx = 0;
if withCamera
...
if withRecording
    ...
    if lags_imec >= 0; ImecNI_imec = imec_idx(1); ImecNI_ni = ni_idx(1+lags_imec);
    else; ImecNI_imec = imec_idx(1-lags_imec); ImecNI_ni = ni_idx(1); end
    ImecNI_niIdx = max([1+lags_imec,1]); ImecNI_imecIdx = max([1-lags_imec,1]);
    figure; stem(l_imec,r_imec)
end
...
if ~(withRecording || withPhotometry || withCamera) % Only nidq
    ...
    firstSamp = [ImecNI_ni,CamNI_ni,LJNI_ni];
    firstIdx = [ImecNI_niIdx,CamNI_niIdx,LJNI_niIdx];
    ...
    if system ~= 1 % if some system comes online after NI
        ...
        if withRecording
            diffIdx = firstIdx(system) - ImecNI_niIdx; 
            ImecNI_imec = imec_idx(ImecNI_imecIdx + diffIdx);
        end
    end
end
...
if withRecording; params.sync.commonStartRecording = ImecNI_imec; end
...
if withRecording
    figure;
    subplot(1,2,1); plot(timeImec); title('Imec time in sec');
    subplot(1,2,2); plot(diff(timeImec)); title('d(timeImec)');
    
    figure; 
    subplot(1,3,1); plot(timeNI); title('NI time in sec');
    subplot(1,3,2); plot(diff(timeNI)); title('d(timeNI)');
...
if withRecording
    ...
    save(strcat(session.path,'\','sync_',sessionName),'timeImec','-append');
end
...
if withPhotometry
    ...
    params.photometry.t0_aligned = filtTimes(LJNI_lj); 
