...
unsigned long UnitRewardSize = 12; // reward size of 1ul
...
int RewardProbRange[2] = {1,11}; //{0, 50};
int PunishProbRange[2] = {11,20}; //{51, 70};
int ToneProbRange[2] = {-51,-60}; //{71, 80};
int StimProbRange[2] = {21,100}; //{81, 100};
int PunishStimProbRange[2] = {101, 110};

// Maximum number of events
int MaxStimNumber = 30;

// Opto stim params
boolean RedStim = false; // 1 means using red to stim, 0 means using blue to stim
int StimTotalPulseNum = 1; // number of pulses per pattern
unsigned long StimPulseDuration = 20000; // Total duration of each pulse within a stimulation
unsigned long StimPulseFreq = 2; //For single 500ms pulse
unsigned long StimTotalDuration = 20000; // Total duration of each stimulation
...
unsigned long BluePulseDuration = 200; // Total duration of each pulse within a stimulation
unsigned long BluePulseFreq = 2; //For single 500ms pulse
unsigned long BlueStimDuration = 200; // Total duration of each stimulation
...
unsigned long ITIMax = 60000;
unsigned long ITIMin = 60000;
...
void loop() {
  ...
  switch (state) {
    ...
    case OutcomeDelivery:
      // Stop if MaxStimNumber is reached
      if (OptoNum >= MaxStimNumber){
        state = 0;
      }
