...
unsigned long UnitRewardSize = 16; // reward size of 1ul
...
int RewardProbRange[2] = {1,10}; //{0, 50};
int PunishProbRange[2] = {-41,-60}; //{51, 70};
int ToneProbRange[2] = {-61,-70}; //{71, 80};
int StimProbRange[2] = {11,40}; //{81, 100};
int PairProbRange[2] = {41,70};
int BlueRedProbRange[2] = {71, 100};
  ...
  switch (state) {
    ...
    case OutcomeDelivery:
      ...
      if (Current_ITI > ITI && Actual_ITI > trialITIMin) {
        ...
        if (trialRandomProb >= RewardProbRange[0] && trialRandomProb <= RewardProbRange[1]) {
        ...
        } else if (trialRandomProb >= PairProbRange[0] && trialRandomProb <= PairProbRange[1]){
          Opto_start = millis();
          Cue_start = millis();
          ...
          Serial.print("Pair stim & tone: ");
          Serial.print(pairNum);
          Serial.print("\t");
          Serial.print("Time: ");
          Serial.println(millis() / 1000.0);

        } else if (trialRandomProb >= BlueRedProbRange[0] && trialRandomProb <= BlueRedProbRange[1]){
          Opto_start = millis();
          
          giveRedOpto();
          giveBlueOpto();
          
          Serial.print("Pair blue & red stim: ");
          Serial.print(BlueRedNum);
          Serial.print("\t");
          Serial.print("Time: ");
          Serial.println(millis() / 1000.0);
        }
...
case EndOutcomeDelivery:
  ...
  if (getTone == 1){
    if ((Outcome_off - Cue_start) >= ShortToneDuration){
      noTone(Speaker);
      digitalWrite(SpeakerLeft_copy, LOW);
  ...
  if (getReward == 0 && getPunish == 0 && getTone == 0) {
    printTrials();
