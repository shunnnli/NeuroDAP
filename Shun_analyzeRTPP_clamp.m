...
animalPaths = uipickfiles( ...
    'FilterSpec', osPathSwitch('/Volumes/Neurobio/MICROSCOPE/Shun/Project clamping/Recordings'), ...
...
for animalIdx = 1:nAnimals
    ...
    for typeIdx = 1:numel(uniqueTypesThisAnimal)
        ...
        stimulatedMaskThisType = typeMask & ...
            (contains(sessionConditions, "stim", IgnoreCase=true) | ...
             contains(sessionConditions, "clamp", IgnoreCase=true));
    ...
    for sessionIdx = 1:nSessions
        barColor = ctrlColor;
        if any(contains(string(sessions(sessionIdx).condition), ["stim","clamp"], 'IgnoreCase', true), 'all')
            barColor = stimColor;
        end
...
groupSummaryCSV = fullfile(groupOutputFolder, 'animals-summary.csv');
writetable(animalTypeSummaryTable, groupSummaryCSV);
disp(['Saved animal summary CSV: ' groupSummaryCSV]);

sessionSummaryCSV = fullfile(groupOutputFolder, 'session-summary.csv');
writetable(allSessionSummaryTable, sessionSummaryCSV);
disp(['Saved session summary CSV: ' sessionSummaryCSV]);
