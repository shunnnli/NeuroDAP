...
% powerAtKnobBlue = 100; %(new driver at max) %140 (old blue driver at min)
% powerAtKnobGreen = 123; %input('Calibrated green LED power (uW): ');
freqMod = false;
LEDpowerblue = 1.25; % 2 * (0.5);
LEDpowergreen = 0;
colorarray = [0 1 0;1 0 0];
looplength = [12,9];
% p = gcp;
save(strcat(session_path,'info.mat'),'samplerate','looplength','LEDpowerblue','LEDpowergreen','script_used','freqMod');
...
% try
    ...
    if freqMod == true
        ...
        % OUT1 = 565nm @ 223 Hz  20 uW power 
        LabJack.LJM.eWriteName(handle, 'STREAM_OUT1_LOOP_SIZE', 9);
        LabJack.LJM.eWriteName(handle, 'STREAM_OUT1_BUFFER_F32', 0.5*LEDpowergreen);  
        LabJack.LJM.eWriteName(handle, 'STREAM_OUT1_BUFFER_F32', 0.5*LEDpowergreen); 
        LabJack.LJM.eWriteName(handle, 'STREAM_OUT1_BUFFER_F32', 0.5*LEDpowergreen); 
        LabJack.LJM.eWriteName(handle, 'STREAM_OUT1_BUFFER_F32', 0.5*LEDpowergreen);  
        LabJack.LJM.eWriteName(handle, 'STREAM_OUT1_BUFFER_F32', 0.5*LEDpowergreen);  
