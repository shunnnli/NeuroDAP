...
%          '',...
%          ['session: ', recordingConfig.sessionName],...
...
session = strcat(recordingConfig.sessionName,'_g0');
...
while ~isfile(ni_file_path) && spikeGLX
    waitdlg = warndlg('NIDAQ recording not started, please press ENABLE RECORDING on SpikeGLX!', 'OK');
    waitfor(waitdlg);
end
