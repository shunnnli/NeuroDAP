function analyzeDMDSearch_Paolo(curCell, searchIdx, options)
...
for d = 1:nDepth
    ...
    for t = 1:4^curDepth
        ...
        % Plot thresholds
        if search_vhold > -10; yline(Ithres,'--',color=red,Alpha=0.5);
        elseif search_vhold < -50; yline(Ethres,'--',color=blue,Alpha=0.5);
        else; yline(Ithres,'--',color=red,Alpha=0.5); yline(Ethres,'--',color=blue,Alpha=0.5);
        end
    ...
    if find(~hotspot_spotIdx)
        xline(spotMax_avg(~hotspot_spotIdx),':',color=blueWhiteRed(400,:)); hold on
        xline(spotMin_avg(~hotspot_spotIdx),':',color=blueWhiteRed(100,:)); hold on
    end
    if find(hotspot_spotIdx)
        xline(spotMax_avg(hotspot_spotIdx),'--',color=red); hold on
        xline(spotMin_avg(hotspot_spotIdx),'--',color=blue); hold on
    end
    xline(Ethres,'-',color=blue,Label='Exci. threshold',LineWidth=5,LabelHorizontalAlignment='left'); hold on
    xline(Ithres,'-',color=red,Label='Inhi. threshold',LineWidth=5); hold on; box off;
    xlabel('Current (pA)');
