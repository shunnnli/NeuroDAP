...
NIWindow = params.sync.commonStartNI : params.sync.commonStartNI+total_ds_samples*NISamplesPerBin-1;
go_ds = squeeze(sum(...
    reshape(leftTone(NIWindow), 1, NISamplesPerBin, total_ds_samples), ...
    2))>0;
nogo_ds = squeeze(sum(...
    reshape(rightTone(NIWindow), 1, NISamplesPerBin, total_ds_samples), ...
    2))>0;
lick_ds = squeeze(sum(...
    reshape(rightLick(NIWindow), 1, NISamplesPerBin, total_ds_samples), ...
    2))>0;
reward_ds = squeeze(sum(...
    reshape(rightSolenoid(NIWindow), 1, NISamplesPerBin, total_ds_samples), ...
    2))>0;
...
% of the last cue is after the end of NIWindow (meaning that the number of original trials 
% (in "trials" table)is greater than the the number of downsampled trials (allTrials_ds))

if height(trials) > height(allTrials_ds)
    newFirstTrial = find(trials.CueTime > params.sync.commonStartNI,1);
    newLastTrial = find(trials.CueTime < NIWindow(end));
