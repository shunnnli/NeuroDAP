     ...
     for iSelectedCell = 1:numel(selectedCells)  
        ...
        if isequal(currentCell.name, selectedCells(iSelectedCell))
            ...
            for iRun = 1:numel(existingRuns)
                ...
                if isempty(regexp(Datastruct.state.randomSearchEpoch, '[a-zA-Z0-9]', 'once'))%
                    
                    DataStruct.searchDepth = NaN;
                    DataStruct.searchRepetition = NaN;
                    ...
                    runSearchDepth = string(headerStringStructure.state.randomSearchDepth);%
                    runSearchDepth = regexp(runSearchDepth, '\d+', 'match');
                    runSearchRepetition = string(headerStringStructure.state.randomSearchRepetition);%
