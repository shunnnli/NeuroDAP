...
%% basic initialization and gui stuff
	...
	prepath = uigetdir('','Select the path with the data folders');
	...
	savepath=uigetdir('', 'Select output path');
	savepath=[savepath '/'];
...
%% set up the variables to process data
	...
	pulseListByCyclePosition=[-100 -50 0:20:300];
	...
	maxRestSD=150; % max SD of the resting voltage to pass QC
 	minRm=20; % min Rm to pass QC
 	maxRm=1000; % max Rm to pass QC
	
	maxVm=-30; % max Vm for inclusion and to pass QC
	minVm=-100; % min Vm for inclusion and to pass QC
	
	initialDeltaVmSweeps=5; % How many of the first sweeps should we use to 
							% determine the starting Vm
	maxDeltaVm=20; % How far can the Vm move from the above value and still be included
	maxFractionalDeltaRm=4.5; % What fractional change in the Rm will be accept
	
	minFractionGoodToKeepCell=0.9;	% What fraction of the traces need to pass QC for
									% the cell to pass QC
...
%		newName=[newCell.mouseID '_' num2str(newCell.cellID)];
		newName=['c_' newCell.Folder '_' num2str(newCell.cellID)];
		newCell.newName=newName;
		disp(['Loaded data for cell #' num2str(cellCounter) ': ' newName ' from ' fullpathshort]);
	...
	% examine resting potential, RC
		...
		for sCounter=1:nAcq
			...
			if ~failedLoad
				...
				newCell.pulsePattern(sCounter)=headerValue('state.phys.internal.pulseToUse0', 1);
				...
				elseif (pulseAmplitudeMode==4)
					deltaI=phUtil_parsePulsePatternString(headerValue('state.phys.internal.pulseString_ao0'), 'amplitude');
					if isnan(deltaI)
						disp(['**** Error extracting pulse amplitude.  Set to nan']);
						deltaI=nan;
					end
				end
				...
				if isempty(checkPulseStart)
					...
					if checkPulseStart>pulseStart % the RC check comes late
						...
						notPulse=[SR(1, checkPulseStart-1) SR(checkPulseEnd+50, pulseStart-10)]; 
					end
		...
		%% Run through the traces and analyze the data	
			...
			disp(['Processing data for cell #' num2str(cellCounter) ': ' newName]);

			for sCounter=acqToAna
				disp(['    Analying sweep #' num2str(sCounter) ': ' newCell.acq{sCounter}.UserData.name]);
				...
				if deltaI<0 % if it was a hyperpolarizing pulse, check for rebound APs
		...
		%% plot the rejected traces
			...
			for sCounter=badTraces
                if ~isempty(newCell.acq{sCounter})
				    acqData=newCell.acq{sCounter}.data;
				    acqEndPt=length(acqData)-1;
		...
		%% plot the good traces	
			...
			if nGood>denom*minFractionGoodToKeepCell
				...
				disp([newCell.newName ' rejected because only ' ...
					num2str(nGood) ' of ' num2str(denom) ' traces pass QC ']);
				newCell.QC=0;
				set(gcf, 'name', [newName ' REJECTED'])
			end
