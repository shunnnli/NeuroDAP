...
for signal = 1:length(options.signalRange)
    ...
    for i = 1:length(signalIdx)
        ...
        % if ~isfield(database,'session') && ~strcmpi(options.sessionRange,'All')
        ...
        %         startTrial = row.options.startIdx.session(sessionIdx(s));
        %         if s == length(sessionIdx); endTrial = size(rowData,1);
        %         else; endTrial = row.options.startIdx.session(sessionIdx(s+1)); end
