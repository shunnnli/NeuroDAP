...
% Find water lick within stimulation periods
blue_starts = bluePatterns.onset(:);
blue_ends = blue_starts + round(bluePatterns.duration_s(:) * params.sync.behaviorFs);

% Keep licks that occur inside any blue interval
inBlue = any(waterLickIdx(:) >= blue_starts' & waterLickIdx(:) < blue_ends', 2);
waterLickIdx_laser = waterLickIdx(inBlue);    % indices of licks during blue laser
waterLickIdx_ctrl = waterLickIdx(~inBlue);
...
if exist(analysisFile, 'file')
    ...
    if ~exist('analysis','var') || options.redo
        analysis = struct('animal',{},'date',{},'session',{},'system',{},'name',{},...
                          'type',{},'pwm',{},'duration',{},'data',{},'timestamp',{});
    end
else
    analysis = struct('animal',{},'date',{},'session',{},'system',{},'name',{},...
                      'type',{},'pwm',{},'duration',{},'data',{},'timestamp',{});
    ...
    for photometry = 1:nSignals
        ...
        if ~isempty(bluePatterns)
            ...
            for ci = 1:numel(durList)
                ...
                [airpuffTraces,timestamp] = plotTraces(airpuffIdx, timeRange, signal, [0 0 0], params, ...
                                   signalFs=finalFs, signalSystem=system);
        ...
        if ~isempty(redPatterns)
            ...
            for ci = 1:numel(durList)
                ...
                [waterTraces,timestamp] = plotTraces(waterLickIdx_ctrl, timeRange, signal, [0 0 0], params, ...
                                   signalFs=finalFs, signalSystem=system);
        ...
        if ~isempty(waterLickIdx_laser)
            initializeFig(0.3,0.6);
            % Black reference trace: rewarded licks
            [waterTraces,timestamp] = plotTraces(waterLickIdx_ctrl, timeRange, signal, bluePurpleRed(1,:), params, ...
                               signalFs=finalFs, signalSystem=system);
            analysis(end+1) = struct( ...
                    'animal',   animal,...
                    'date',     date,...
            ...
            [waterTraces,timestamp] = plotTraces(waterLickIdx_laser, timeRange, signal, addOpacity(bluePurpleRed(1,:),0.5), params, ...
                               signalFs=finalFs, signalSystem=system);
            plotEvent('',0,color=bluePurpleRed(1,:));
            analysis(end+1) = struct( ...
                    'animal',   animal,...
                    'date',     date,...
                    'session',  options.outputName,...
