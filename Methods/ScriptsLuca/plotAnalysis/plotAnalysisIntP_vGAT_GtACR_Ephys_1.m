...
experimentName = 'vGAT_GtACR_Ephys_1_240319';
...
%     meanAllData = [meanAllData, nanmean(vertcat(allData{1,iRunGroup}{:}))];
%     stdAllData = [stdAllData, nanstd(vertcat(allData{1,iRunGroup}{:}))];
%     positionOptoParameterValue = find(sort(queryOptoParameterValues) == queryOptoParameterValues(iRunGroup));
    %histogram(horzcat(allData{1,iRunGroup}{:})',8)
    [dataCount,~] = histcounts(horzcat(allData{1,iRunGroup}{:})',6);
    binCenters = edges(2:end) - (edges(2)-edges(1))/2;
    plot(binCenters, dataCount, 'Color', cMap(iRunGroup,:), 'LineWidth', 2)
    hold on
    cMap(iRunGroup,:)
%     hscatter = scatter(positionOptoParameterValue, vertcat(allData{1,iRunGroup}{:}),'k','filled');
%     hold on;
...
trace = selectedRunGroupsAnalysisTable.trace{79,1};
...
% % 
% % xValues = linspace(1,numel(queryOptoParameterValues),numel(queryOptoParameterValues));
% % bar(xValues,meanAllData,'FaceColor',[0 0.4470 0.7410],'BarWidth', 0.5)
% % errorbar(xValues,meanAllData,stdAllData,'.','Color',[0 0 0],'LineWidth', 1.2, 'CapSize', 14)
% % 
% % hScatter = findobj(gca, 'Type', 'scatter');
% % uistack(hScatter, 'top');
% % 
