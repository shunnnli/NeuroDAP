...
overallExperimentName = 'DCN_Inputs_Ephys';
experimentName = overallExperimentName;
mouseExperimentNames = {};
mouseExperimentNames{1} = 'DCN_SNr_Ephys_1_1';
mouseExperimentNames{2} = 'DCN_ZI_Ephys_1_2';

% Indicate cells to select
mouseExperimentSelectedCells = {};
mouseExperimentSelectedCells{1} = [1,2,3,4,6,7];
mouseExperimentSelectedCells{2} = [1,2,4,7,8,9];
...
cat3_queryOptoParameter =  'amplitudeBlue';
queryRunFeature = 'heightPulsePeak';

plotBaseName = [experimentName '_' queryRunFeature]; 
queryOptoParameterValuesOverall = [5];

cat1_selectedCellsIDs = [8,9];
cat2_selectedCellsIDs = [8,9];
cat3_selectedCellsIDs = [1,2,3,4,6,7,11,14,15,16];
...
cat3_selectedRunGroups = find(strcmp(runGroupsParametersTable.activeChannels,'ao0, ao1') & cell2mat(runGroupsParametersTable.holdingVoltage) == -70 & cell2mat(runGroupsParametersTable.amplitudeBlue) == 5 & strcmp(runGroupsParametersTable.functionNameBlue,'dmdOutputRect(1, 1, 1, 1, 1, 1)'));
...
cat1_selectedCellsIDs = [8,9];
cat2_selectedCellsIDs = [1,2,3,4,6,7,11,14,15,16];%1,2,3,4,6,7,11,14,15,16
...
cat1_selectedCellsIDs = [8,9];
cat2_selectedCellsIDs = [1,2,3,4,6,7,11,14,15,16];
