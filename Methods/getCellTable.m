...
for i = 1:length(animalList)
    ...
    for c = 1:size(cellList,1)
        ...
        if options.calculate
            ...
            stats.summary.EPSC.peakAvg = mean(cell2mat(stats.response.min(EPSCrows)),'all');
            stats.summary.IPSC.peakAvg = mean(cell2mat(stats.response.max(IPSCrows)),'all');
            stats.summary.EPSC.aucAvg = mean(cell2mat(stats.response.auc(EPSCrows)),'all');
            stats.summary.IPSC.aucAvg = mean(cell2mat(stats.response.auc(IPSCrows)),'all');
        end
