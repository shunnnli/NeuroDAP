...
if (withPhotometry || options.withPhotometryNI) && (options.reloadAll || options.reloadLJ)
    ...
    if withPhotometry
        for i = 1:labjack.nSignals
            ...
            if labjack.mod(i)
                processed = demodulatePhotometry(labjack.raw(i,:),...
                ...
                % Downsample
                processed = downsamplePhotometry(labjack.raw(i,:),...
                            targetFs=options.downsampleFs,...
                            originalFs=params.sync.labjackFs,...
    ...
    if options.withPhotometryNI == true
        ...
        if options.photometryNI_mod
            processed = demodulatePhotometry(photometry_raw,...
                            targetFs=options.downsampleFs,...
                            originalFs=nidq.Fs,...
            ...
            % Downsample
            processed = downsamplePhotometry(photometry_raw,...
    ...
    if length(opts.VariableNames) == 8
        ...
        if isempty(skipped_frame)
            ...
            for i = 1:size(traces,1)
                processed_cam = downsamplePhotometry(traces{i},...
                                targetFs=options.downsampleFs,...
...
elseif ~(withRecording || withNI || withCamera) % Only Labjack
    ...
    firstSamp = [Imec_inBaseline,Cam_inBaseline,LJ_inBaseline,LFP_inBaseline];
    firstIdx = [Imec_inBaselineIdx,Cam_inBaselineIdx,LJ_inBaselineIdx,LFP_inBaselineIdx];
    if strcmpi(session.baselineSystem,'NI')
