...
for i = 1:length(clusterList)
    ...
    temp_2 = nanstd(temp, 0, 1) ./ sqrt(size(temp, 1)); % SEM
    
    % Skip neurons with low firing rates
    if sum(temp_1) > 10
        % Smooth the firing rate
        smoothed_fr = smoothdata(temp_1, 'movmean', 5);
        
        % Apply significance mask
        smoothed_fr_significant_only = smoothed_fr;
        smoothed_fr_significant_only(~significance) = NaN;

        % Plot area with SEM
        h = area(t, [(smoothed_fr - temp_2); 2 * temp_2]', 0);
        set(h(1), 'Edgecolor', 'none', 'FaceColor', 'none');
        set(h(2), 'Edgecolor', 'none', 'FaceColor', colors(mod(i - 1, numColors) + 1), 'FaceAlpha', 0.3);
        
        % Plot the smoothed firing rate and scatter the significant points
        plot(t, smoothed_fr, '-', 'LineWidth', 1, 'Color', colors(mod(i - 1, numColors) + 1));
        hold on;
        scatter(t, smoothed_fr_significant_only, 17, 'filled');
        
        % Add neuron labels if requested
        if textOn
            [~, x_max] = max(abs(smoothed_fr));
            text(t(x_max), smoothed_fr(x_max), num2str(ap.goodClusters(neuron)), 'FontSize', 8);
        end
