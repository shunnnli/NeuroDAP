function analyzeSessions_EPLHbPair(sessionpath,options)
...
elseif ~isfield(params.session,'task') && ~isfield(options,'task')
    if contains(sessionTask,["Random","H"],IgnoreCase=false)
        options.task = 'random';
        params.session.task = options.task;
    elseif contains(sessionTask,"Simultaneous",IgnoreCase=false)
        options.task = 'simultaneous';
        params.session.task = options.task;
    elseif contains(sessionTask,"Sequential",IgnoreCase=false)
        options.task = 'sequential';
        params.session.task = options.task;
    else
...
if ~exist('trials','var') || options.redo
    ...
    elseif contains(options.task,'simultaneous')
        if options.includeOtherStim
            [allTrials,~] = getTrials(find(leftTone),optoStim1,optoStim2,...
                                 waterIdx,airpuffIdx);
        else
...
if strcmp(options.task,'random')
    ...
    if options.combineOmission
        if contains(options.task,'simultaneous')
            if options.performing
                stimTrials = trials{trials.isTone == 0 & trials.isStim == 1 & trials.isStim2 == 0 & trials.performing == 1,["TrialNumber","CueTime","OutcomeTime","ENL"]};
                EPLHbTrials = trials{trials.isTone == 0 & trials.isStim == 1 & trials.isStim2 == 1 & trials.performing == 1,["TrialNumber","CueTime","OutcomeTime","ENL"]};
                pairTrials = trials{trials.isTone == 1 & trials.isStim == 1 & trials.isStim2 == 1 & trials.performing == 1,["TrialNumber","CueTime","OutcomeTime","ENL"]};
                toneTrials = trials{trials.isTone == 1 & trials.isStim == 0 & trials.isStim2 == 0 & trials.performing == 1,["TrialNumber","CueTime","OutcomeTime","ENL"]};
                stimIdx = stimTrials(:,2);
                pairIdx = pairTrials(:,2);
                EPLHbIdx = EPLHbTrials(:,2);
                toneIdx = toneTrials(:,2);
                stimOmissionIdx = []; pairOmissionIdx = []; stimToneOmissionIdx = [];
            else
                stimTrials = trials{trials.isTone == 0 & trials.isStim == 1 & trials.isStim2 == 0,["TrialNumber","CueTime","OutcomeTime","ENL"]};
                EPLHbTrials = trials{trials.isTone == 0 & trials.isStim == 1 & trials.isStim2 == 1,["TrialNumber","CueTime","OutcomeTime","ENL"]};
                pairTrials = trials{trials.isTone == 1 & trials.isStim == 1 & trials.isStim2 == 1,["TrialNumber","CueTime","OutcomeTime","ENL"]};
                toneTrials = trials{trials.isTone == 1 & trials.isStim == 0 & trials.isStim2 == 0,["TrialNumber","CueTime","OutcomeTime","ENL"]};
                stimIdx = stimTrials(:,2);
                pairIdx = pairTrials(:,2);
                EPLHbIdx = EPLHbTrials(:,2);
                toneIdx = toneTrials(:,2);
        ...
        elseif contains(options.task,'sequential')
            if options.performing
        ...
        if contains(options.task,'simultaneous')
            if options.performing
                stimTrials = trials{trials.isTone == 0 & trials.isStim == 1 ...
                    & trials.isReward == 1 & trials.isStim2 == 0 & trials.performing == 1,["TrialNumber","CueTime","OutcomeTime","ENL"]};
                stimOmissionTrials = trials{trials.isTone == 0 & trials.isStim == 1 ...
                    & trials.isReward == 0 & trials.isStim2 == 0 & trials.performing == 1,["TrialNumber","CueTime","OutcomeTime","ENL"]};
                pairTrials = trials{trials.isTone == 1 & trials.isStim == 1 ...
                    & trials.isReward == 1 & trials.isStim2 == 1 & trials.performing == 1,["TrialNumber","CueTime","OutcomeTime","ENL"]};
                pairOmissionTrials = trials{trials.isTone == 1 & trials.isStim == 1 ...
                    & trials.isReward == 0 & trials.isStim2 == 1 & trials.performing == 1,["TrialNumber","CueTime","OutcomeTime","ENL"]};
                EPLHbTrials = trials{trials.isTone == 0 & trials.isStim == 0 ...
                    & trials.isReward == 1 & trials.isStim2 == 1 & trials.performing == 1,["TrialNumber","CueTime","OutcomeTime","ENL"]};
                EPLHbOmissionTrials = trials{trials.isTone == 0 & trials.isStim == 0 ...
                    & trials.isReward == 0 & trials.isStim2 == 1 & trials.performing == 1,["TrialNumber","CueTime","OutcomeTime","ENL"]};
                toneTrials = trials{trials.isTone == 1 & trials.isStim == 0 ...
                    & trials.isReward == 0 & trials.isStim2 == 0 & trials.performing == 1,["TrialNumber","CueTime","OutcomeTime","ENL"]};
                toneOmissionTrials = trials{trials.isTone == 1 & trials.isStim == 0 ...
                    & trials.isReward == 0 & trials.isStim2 == 0 & trials.performing == 1,["TrialNumber","CueTime","OutcomeTime","ENL"]};
                ...
                EPLHbIdx = EPLHbTrials(:,2);
                EPLHbOmissionIdx = EPLHbOmissionTrials(:,2);
                toneIdx = toneTrials(:,2);
                toneOmissionIdx = toneOmissionTrials(:,2);
            else
                stimTrials = trials{trials.isTone == 0 & trials.isStim == 1 ...
                    & trials.isReward == 1 & trials.isStim2 == 0,["TrialNumber","CueTime","OutcomeTime","ENL"]};
                stimOmissionTrials = trials{trials.isTone == 0 & trials.isStim == 1 ...
                    & trials.isReward == 0 & trials.isStim2 == 0,["TrialNumber","CueTime","OutcomeTime","ENL"]};
                pairTrials = trials{trials.isTone == 1 & trials.isStim == 1 ...
                    & trials.isReward == 1 & trials.isStim2 == 1,["TrialNumber","CueTime","OutcomeTime","ENL"]};
                pairOmissionTrials = trials{trials.isTone == 1 & trials.isStim == 1 ...
                    & trials.isReward == 0 & trials.isStim2 == 1,["TrialNumber","CueTime","OutcomeTime","ENL"]};
                EPLHbTrials = trials{trials.isTone == 0 & trials.isStim == 0 ...
                    & trials.isReward == 1 & trials.isStim2 == 1,["TrialNumber","CueTime","OutcomeTime","ENL"]};
                EPLHbOmissionTrials = trials{trials.isTone == 0 & trials.isStim == 0 ...
                    & trials.isReward == 0 & trials.isStim2 == 1 ,["TrialNumber","CueTime","OutcomeTime","ENL"]};
    ...
    if contains(options.task,'simultaneous')
        toneIdx = EPLHbIdx;
        analysisEvents = {waterIdx,waterLickIdx,EPLHbIdx,stimIdx,pairIdx,airpuffIdx,baselineIdx,optoStim2};
        eventTrialNum = {findTrials(waterIdx,trials),findTrials(waterLickIdx,trials),...
                        EPLHbTrials(:,1),stimTrials(:,1),pairTrials(:,1),...
                        findTrials(airpuffIdx,trials),findTrials(baselineIdx,trials),findTrials(optoStim2,trials)};
        if strcmp(params.stim.color,'red')
            analysisLabels = {'Water','Rewarded licks','EP & LHb stim','EP stim','Pair','Airpuff','Baseline','Blue stim'};
        else
            analysisLabels = {'Water','Rewarded licks','EP & LHb stim','EP stim','Pair','Airpuff','Baseline','Red stim'};
        end
    elseif contains(options.task,'sequential')
        EPLHbIdx = [];
    ...
    save(strcat(sessionpath,filesep,'behavior_',options.outputName),'allTrials',...
        'waterIdx','waterLickIdx','airpuffIdx','EPLHbIdx','stimIdx','pairIdx','toneIdx',...
        'baselineIdx',...
        '-append');
end
        ...
        if strcmp(options.task,'random')
            ...
            if contains(options.task,'simultaneous')
                if strcmp(params.stim.color,'red')
                    ...
                    omissionIdxes = {stimOmissionIdx, pairOmissionIdx,stimToneOmissionIdx,[],[],[]};
                    labels = {'Stim','Pair','EP & LHb stim','Water','Airpuff','Blue stim'};
                    ...
                    eventIdxes = {stimIdx,pairIdx,toneIdx,waterLickIdx,airpuffIdx,optoStim2};
                    omissionIdxes = {stimOmissionIdx, pairOmissionIdx,stimToneOmissionIdx,[],[],[]};
                    labels = {'Stim','Pair','EP & LHb stim','Water','Airpuff','Red stim'};
                    eventDurations = [stimDuration_cue/1000,0.5,0.5,0,0.02,stimDuration_other/1000];
                    groupSizes = [10,10,10,30,30,10];
                    longTimeRange = options.longTimeRange;
                    shortTimeRange = options.shortTimeRange;
