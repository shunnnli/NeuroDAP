...
addpath(genpath('/Users/shunli/Downloads/Sabatini lab/npy-matlab'))
addpath(genpath('/Users/shunli/Downloads/Sabatini lab/dPCA-matlab'))
addpath(genpath('/Users/shunli/Downloads/Sabatini lab/spikes'))
...
% sessionName = '20220427 SJ498-R_g0';
% sessionName = '20220428 SJ498-R_g0';
...
ap.goodClusters=clusterLabel.cluster_id(find(clusterLabel.KSLabel=='good')); % Good units labeled by hand
...
% allTrials(leftTone) = 1;
% allTrials(rightTone) = 2;
...
% Plot left cue
plotLeft = true; spikeRate = leftCueSpikeRate;
figure; drawPSTH(plotLeft,timeRange,spikeRate,clusterList,textOn,ap,'map')
subtitle(['Time bin: ',num2str(binSize*1000),' ms; '...
      'Total time analyzed: ',num2str(session.nBlocks*session.blockTime),'s']);
      
% Plot right cue
plotLeft = false; spikeRate = rightCueSpikeRate;
figure; drawPSTH(plotLeft,timeRange,spikeRate,clusterList,textOn,ap,'map')
subtitle(['Time bin: ',num2str(binSize*1000),' ms; '...
      'Total time analyzed: ',num2str(session.nBlocks*session.blockTime),'s']);
  
% Plot left omission cue
plotLeft = true; spikeRate = leftOmissionSpikeRate;
figure; drawPSTH(plotLeft,timeRange,spikeRate,clusterList,textOn,ap,'map')
...
plotLeft = false; spikeRate = rightOmissionSpikeRate;
figure; drawPSTH(plotLeft,timeRange,spikeRate,clusterList,textOn,ap,'map')
...
plotLeft = true; eventIdx = find(allTrials==1);
figure; drawPeriEventLicks(plotLeft,timeRange,eventIdx,...
                        leftLick,rightLick,nidq,timeNI)

% Plot right-cue-triggered licks
plotLeft = false; eventIdx = find(allTrials==2);
figure; drawPeriEventLicks(plotLeft,timeRange,eventIdx,...
                        leftLick,rightLick,nidq,timeNI)
                    
% Plot left-omission-triggered licks
plotLeft = true; eventIdx = find(omissionTrials==1);
figure; drawPeriEventLicks(plotLeft,timeRange,eventIdx,...
                        leftLick,rightLick,nidq,timeNI) 
                    
% Plot right-omission-triggered licks
plotLeft = false; eventIdx = find(omissionTrials==2);
figure; drawPeriEventLicks(plotLeft,timeRange,eventIdx,...
                        leftLick,rightLick,nidq,timeNI)                    
...
plotLeft = true; spikeRate = leftCueSpikeRate; eventIdx = find(allTrials==1);
yyaxis left; drawPeriEventLicks(plotLeft,timeRange,eventIdx,...
                            leftLick,rightLick,nidq,timeNI);
yyaxis right; drawPSTH(plotLeft,timeRange,spikeRate,clusterList,textOn,ap,'map');

% Plot right cue trials
set(figure,'defaultAxesColorOrder',[[0.9290 0.6940 0.1250]; [0 0.4470 0.7410]]);
plotLeft = false; spikeRate = rightCueSpikeRate; eventIdx = find(allTrials==2);
yyaxis left; drawPeriEventLicks(plotLeft,timeRange,eventIdx,...
                            leftLick,rightLick,nidq,timeNI);
yyaxis right; drawPSTH(plotLeft,timeRange,spikeRate,clusterList,textOn,ap,'map');
...
set(figure,'defaultAxesColorOrder',[[0.9290 0.6940 0.1250]; [0 0.4470 0.7410]]);
plotLeft = true; spikeRate = leftCueSpikeRate; eventIdx = find(omissionTrials==1);
yyaxis left; drawPeriEventLicks(plotLeft,timeRange,eventIdx,...
                            leftLick,rightLick,nidq,timeNI);
yyaxis right; drawPSTH(plotLeft,timeRange,spikeRate,clusterList,textOn,ap,'map');
title(['Averged PSTH of left omission trial per neuron']);
...
set(figure,'defaultAxesColorOrder',[[0.9290 0.6940 0.1250]; [0 0.4470 0.7410]]);
plotLeft = false; spikeRate = rightCueSpikeRate; eventIdx = find(omissionTrials==2);
yyaxis left; drawPeriEventLicks(plotLeft,timeRange,eventIdx,...
                            leftLick,rightLick,nidq,timeNI);
yyaxis right; drawPSTH(plotLeft,timeRange,spikeRate,clusterList,textOn,ap,'map');
title(['Averged PSTH of right omission trial per neuron']);
...
for i = 1:nTopPC
    ...
    yyaxis left; drawPeriEventLicks(plotLeft,timeRange,eventIdx,...
                                leftLick,rightLick,nidq,timeNI);
    yyaxis right; drawPSTH(plotLeft,timeRange,spikeRate,Itop,textOn,ap,'r'); hold on
    drawPSTH(plotLeft,timeRange,spikeRate,Ilow,textOn,ap,'b');
...
% Set up time variable
timeRange = [-0.01,0.01]; stimOnIdx = blueLaserON(801:900);
...
x = linspace(timeRange(1),timeRange(2),nBins)*1000;
...
addpath(genpath('/Users/shunli/Downloads/laser_shuffle'));
blueLaserON = find(redLaser);

%% Generate input for laser_shuffle

table_name = strcat('Optotag/optotag_',sessionName,'.csv');
...
spikeTable{1,1} = laserTime(801:900);
...
%% (Unfinished) Generate laser-triggered firing rate for each good cluster

timeRange = [-1,10]; binSize = 0.005; % in sec

npulse_per_stim = 1; stim_per_pattern = 100; nPatterns = 9;
nStim = nPatterns * stim_per_pattern;
blueLaserOnIdx = blueLaserON(701:10:800);
...
[optoSpikes,optoSpikeRate] = getSpikes(timeRange,binSize,blueLaserOnIdx,...
                                    ap,nidq,timeNI);
% optoSpikeRate = optoSpikes / (binSize*length(blueLaserOnIdx));

clusterList = 1:ap.nGoodClusters; textOn = true;

% Plot PSTH for each neuron across trials
figure; plotLeft = true;
drawPSTH(plotLeft,timeRange,optoSpikeRate,clusterList,textOn,ap,'map');
...
% ======================================= %
function [] = drawPeriEventLicks(plotLeft,timeRange,eventIdx,...
                leftLick,rightLick,nidq,timeNI)
...
end % drawPeriEventLicks

% ======================================= %
function [] = drawPSTH(plotLeft,timeRange,...
...
% ======================================= %
function ap = readSpikeData(clusterLabel,spike_clusters,spike_times,ap)
...
% Create separate array for cluster_id of good cluster spikes
goodClusterSpikeIndices=find(ismember(int32(spike_clusters), ap.goodClusters));
...
plot(x,smooth(yMean),'Color',color,'LineWidth',2); hold on
patch('XData',[x, fliplr(x)], 'YData',[yMean-yCI95, fliplr(yMean+yCI95)], ...
