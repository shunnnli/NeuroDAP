...
eye = readmatrix(strcat('D:\Shun\Recordings\',sessionName, '\','eye-pixel-intensity.csv'));
...
goodClusterSpikeIndices=find(ismember(spike_clusters, int32(ap.goodClusters)));
...
timeRange = [-1,5]; binSize = 0.01; % in sec
...
%% Plot eye pixel intensity
eventIdx = find(allTrials==1);
eyeTraces_reward = getEye(timeRange,eventIdx,eye,sync,timeNI,timeCamera);

eventIdx = find(allTrials==2);
eyeTraces_airpuff = getEye(timeRange,eventIdx,eye,sync,timeNI,timeCamera);

figure; 
t = linspace(timeRange(1),timeRange(2),size(eyeTraces,2));
plotCI(t,eyeTraces_reward,colors(1)); hold on
plotCI(t,eyeTraces_airpuff,colors(2));
