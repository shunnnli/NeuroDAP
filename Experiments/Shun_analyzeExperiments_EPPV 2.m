...
%% Make changes to summary for further analysis
...
% Make changes to animals struct
for i = 1:length(summary)
    cur = summary(i).event;
    if strcmpi('Blue stim',cur)
        summary(i).event = 'CoChR';
    end
end
...
xlabel('Time (s)'); ylabel('z-score'); ylim([-1,4]);
...
%% Baseline licking: plot water, airpuff, stim, tone
...
for s = 1:length(signalRange)
    ...
    for i = 1:length(eventRange)
        ...
        combined = combineTraces(animals,timeRange=timeRange,...
                                    ...
                                    signalRange=signalRange{s});
        plotTraces(combined.data{1},combined.timestamp,color=colorList{i},plotShuffled=false);
        xlabel('Time (s)'); ylabel([signalRange{s},' z-score']); ylim([0,10]);
        plotEvent(eventRange{i},eventDuration(i),color=colorList{i});
        legend({[eventRange{i},' (n=',num2str(size(combined.data{1},1)),')']},...
                'Location','northeast');
    end
    % saveFigures(gcf,'Summary_random_licking',...
    %         strcat(resultspath),...
    %         saveFIG=true,savePDF=true);
end
...
saveFigures(gcf,'Summary_CSvsTrialsGrouped_Max',...
        strcat(resultspath),...
        saveFIG=true,savePDF=true);
