...
for i = 1:length(summary)
    ...
    if ~any(strcmpi(cur_animal, {'SL433','SL431','SL432','BiPOLES2'}))
        summary(i).task = 'Reward-Ctrl';
        continue
    end

    % SL433-specific rules
    if strcmpi(cur_animal, 'SL433') && cur_date >= 20260429 && cur_date <= 20260507
        summary(i).task = 'Reward-Clamp-CueDelay';
    elseif strcmpi(cur_animal, 'SL433') && cur_date >= 20260511 && cur_date <= 20260514 && cur_date ~= 20260512
        summary(i).task = 'Reward-Clamp-CueDelayBlock';
    elseif strcmpi(cur_animal, 'SL433') && cur_date >= 20260427 && cur_date <= 20260428 || cur_date == 20260512
        summary(i).task = 'Reward-Unclamp';

    % SL431-specific rules
    elseif strcmpi(cur_animal, 'SL431') && cur_date >= 20260522 && cur_date <= 20260524
        summary(i).task = 'Reward-Clamp-CueDelay';
    elseif strcmpi(cur_animal, 'SL431') && cur_date >= 20260525 && cur_date <= 20260527
        summary(i).task = 'Reward-Clamp-CueDelayBlock';
    elseif strcmpi(cur_animal, 'SL431') && cur_date == 20260520 || cur_date == 20260529
        summary(i).task = 'Reward-Unclamp';

    % BiPOLES2-specific rules
    elseif strcmpi(cur_animal, 'BiPOLES2') && cur_date >= 20260616 && cur_date <= 20260620 || cur_date == 20260626
        summary(i).task = 'Reward-Clamp-CueDelay';
    elseif strcmpi(cur_animal, 'BiPOLES2') && cur_date >= 20260622 && cur_date <= 20260625
        summary(i).task = 'Reward-Clamp-CueDelayBlock';
    elseif strcmpi(cur_animal, 'BiPOLES2') && cur_date == 20260614 || cur_date == 20260615
        summary(i).task = 'Reward-Unclamp';
    else
...
%% Plot DA clamp response (unfinished)
...
for s = 1:length(signalRange)
    ...
    for i = 1:length(eventRange)
        ...
        % plotTraces(combined.data{1},combined.timestamp,color=clampColor,LineStyle='--');
        % 
        % combined = combineTraces(animals,timeRange=timeRange,...
        ...
        %                             trialRange=trialRange,...
        %                             signalRange=signalRange{s});
        % plotTraces(combined.data{1},combined.timestamp,color=unclampColor,LineStyle='--');

        combined = combineTraces(animals,timeRange=timeRange,...
                                    eventRange='Tone (clamp)',...
                                    animalRange=animalPlotted,...
                                    taskRange=taskRange,...
                                    totalTrialRange=totalTrialRange,...
                                    trialRange=trialRange,...
                                    signalRange=signalRange{s});
        plotTraces(combined.data{1},combined.timestamp,color=clampColor,...
            label=['Tone (clamp)',' (n=',num2str(size(combined.data{1},1)),')']);
        ...
        plotTraces(combined.data{1},combined.timestamp,color=unclampColor,...
            label=['Tone (unclamp)',' (n=',num2str(size(combined.data{1},1)),')']);

        xlabel('Time (s)'); ylabel([signalRange{s},' dff']); ylim([-0.1,0.4]);
...
for task = 1:length(taskRange)
    ...
    plotEvent('Tone',.5,color=colorList);
