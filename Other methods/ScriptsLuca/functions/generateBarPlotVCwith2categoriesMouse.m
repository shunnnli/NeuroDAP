function generateBarPlotVCwith2categoriesMouse(cat1_queryOptoParameter, cat2_queryOptoParameter, queryRunFeature, queryOptoParameterValuesOverall, cat1_selectedRunGroupsAnalysisTable, cat2_selectedRunGroupsAnalysisTable, cat1_selectedRunGroups, cat2_selectedRunGroups, cat1_selectedCellsIDs, cat2_selectedCellsIDs, groupByCell, plotBaseName, savePlotPath, savePlots, nSigns)
    ...
    for iSign = 1:nSigns
        ...
        if groupByCell == 1
            ...
            cat1_allTracesToPlot = meansTracesAllCells - mean(meansTracesAllCells(:,:,1:traceDuration/4),3);
        ...
        for iRunGroup = 1:cat2_nSelectedRunGroups
            ...
            if ~isempty(rowsRunGroup)
                ...
                cat2_cellNameData{cat2_groupsCounter} = cell2mat(cat2_selectedRunGroupsAnalysisTable.cellName(rowsRunGroup));
        ...
        if groupByCell == 1
            ...
            cat2_allTracesToPlot = meansTracesAllCells - mean(meansTracesAllCells(:,:,1:traceDuration/4),3);
        ...
        xlim([length(squeeze(cat1_meanTracesToPlot))/2-6000,length(squeeze(cat1_meanTracesToPlot))/2-3000])
        xlabel('Time')
        ylabel('Current [pA]')
        box on;
        set(gca, 'LineWidth', 0.5, 'FontSize', 12, 'FontName', 'Arial');
