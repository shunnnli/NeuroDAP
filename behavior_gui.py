        ...
        try:
            ...
            for name in ("parse_arduino_line", "apply_session_logic"):
                if not callable(getattr(self.module, name, None)):
                    raise ValueError(f"Protocol must define {name}(). See Tutorials/behavior_gui.md.")
...
class BehaviorService:
    ...
    def do_upload(self, settings):
        ...
        self.state()
        try:
            with tempfile.TemporaryDirectory(prefix="behavior-build-") as build:
                common = ["--fqbn", settings.board.strip(), "--build-path", build]
                self._run_cli([cli, "compile", *common, str(sketch)], 600)
                self._run_cli([cli, "upload", *common, "--port", settings.port.strip(), str(sketch)], 120)
...
class BehaviorGUI:
    def __init__(self, root):
        ...
        columns.columnconfigure((0, 1), weight=1, uniform="panels")
        ...
        ttk.Label(right, text="Serial monitoring works without a protocol.\nUDP sends are logged; receipt is not confirmed.\nStop cancels pending sends, not the remote device state.",
                  style="Hint.TLabel", wraplength=425, justify="left").grid(row=5, column=0, columnspan=3, sticky="w", pady=8)
    ...
    def _entry(self, frame, label, key, row, values=None, protocol=False):
        ...
        widget = factory(frame, textvariable=self.vars[key], width=25, **options)
        widget.grid(row=row, column=1, sticky="ew", pady=4)
        (self.protocol_widgets if protocol else self.config_widgets).append(widget)
        return widget

    def settings(self):
        return Settings(**{key: var.get() for key, var in self.vars.items()})
    ...
    def upload(self):
        self.request("upload", self.settings())
    ...
    def save_profile(self):
        ...
        path = filedialog.asksaveasfilename(parent=self.root, defaultextension=".json", filetypes=[("Profile", "*.json")])
        if path:
            try:
    ...
    def load_profile(self):
        from tkinter import filedialog, messagebox
        path = filedialog.askopenfilename(parent=self.root, filetypes=[("Profile", "*.json")])
        if path:
            try:
